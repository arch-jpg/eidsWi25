...
var current_map_node_id: String = ""  # ID of the current active node
var map_scroll_position: Vector2 = Vector2.ZERO  # Saved scroll position
...
var is_boss_fight: bool = false  # Flag to track if current fight is against boss
...
# Map System
func save_map_state(map_nodes: Array[MapNode], current_node: MapNode, scroll_pos: Vector2 = Vector2.ZERO) -> void:
	"""Save the current map state."""
	saved_map_data.clear()
	for node in map_nodes:
		...
		for next_node in node.next_nodes:
			node_data["next_node_ids"].append(next_node.id)
		saved_map_data[node.id] = node_data
	
	if current_node:
		current_map_node_id = current_node.id
	map_scroll_position = scroll_pos
	print("Map state saved with %d nodes" % saved_map_data.size())

func has_saved_map() -> bool:
	"""Check if there is a saved map."""
	return not saved_map_data.is_empty()

func clear_map_state() -> void:
	"""Clear saved map state (e.g., on player death)."""
	saved_map_data.clear()
	current_map_node_id = ""
	map_scroll_position = Vector2.ZERO
	print("Map state cleared")
...
func load_save_data(data: Dictionary):
	...
	current_map = data.get("current_map", "map_system")
